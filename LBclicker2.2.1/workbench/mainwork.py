...
def mouseClick(clickTimes,lOrR,img,img_list,n):
    for j in range(len(img_list)):
        location=locateCenterOnScreen('./assets/'+img,confidence=0.9)
        if location is not None:
            click(location.x,location.y,clicks=clickTimes,interval=0.2,duration=0.2,button=lOrR)
            print("Clicking ",img)
            break
        else:
            print("Try Again ...")
...
def checkpic(pic):
    #检查屏幕上是否出现了一个图像
    if locateOnScreen('./assets/'+pic,confidence=0.9) is not None:
        print('Checked',pic)
        return True
